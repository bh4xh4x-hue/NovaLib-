...
-- ===== Utility =====
local function safeCall(fn, ...) local ok,res = pcall(fn, ...) return ok and res or nil end
local function canWrite() return pcall(function() return writefile ~= nil end) end
local function saveConfigFile(name, tbl) if not canWrite() then return false end local ok,enc = pcall(HttpService.JSONEncode, HttpService, tbl) if not ok then return false end pcall(writefile, name, enc) return true end
local function loadConfigFile(name) if not canWrite() then return nil end if not pcall(readfile,name) then return nil end local txt = readfile(name) local ok,dec = pcall(HttpService.JSONDecode, HttpService, txt) if ok then return dec end return nil end
...
function NovaLib:MakeWindow(opts)
    ...
    do
        ...
        UIS.InputChanged:Connect(function(inp)
            if dragging and inp==dragInput then update(inp) end
        end)
    end
